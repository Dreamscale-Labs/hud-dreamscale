...
import httpx
import pytest
from dreamscale.errors import from_payload

from hud_dreamscale.pooled import RequestFailedError, _error_fields, _http_operation
from tests.test_pooled import Server, predict


async def test_provider_closes_its_private_resolver_on_repeated_cleanup():
    provider = Server().provider()
    resolver = provider._resolver
    assert resolver is not None
    await provider.close()
    await provider.close()
    assert resolver._executor._shutdown
...
def test_error_fields_exclude_messages_and_unbounded_codes(code, expected):
    ...
    assert result["error_type"] == "DreamscaleError"
...
async def test_http_api_rejection_is_not_mislabeled_as_transport_timeout():
    ...
    async with server.provider(
        ...
        with pytest.raises(RequestFailedError):
            await predict(provider)
    post = next(fields for name, fields in events if name == "inference_post")
    assert post["error_type"] == "DreamscaleError" and post["error_code"] == "gateway_overloaded"
    responses = [
        fields
        for name, fields in events
        if name == "provider_http_response" and fields["request_id"]
