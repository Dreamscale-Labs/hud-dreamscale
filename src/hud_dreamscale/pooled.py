...
class PinnedResolver:
    """Resolve gateway hosts off-loop, with TTL caching and stale answers on failure.
...
def _error_fields(exc):
    ...
    code = exc.info.code if isinstance(exc, DreamscaleError) else None
    return {
...
class PooledProvider:
    ...
    async def _recovery_call(self, client, identity, *, operation):
        ...
        except (httpx.TransportError, DreamscaleError) as exc:
            self._event(
                "provider_http_error",
                operation=operation,
                duration_s=time.monotonic() - started,
        ...
        async with asyncio.timeout(self.recovery_timeout):
            while True:
                try:
                    ...
                    except (httpx.TransportError, DreamscaleError):
                        result = None
